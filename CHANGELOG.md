...
- Combobox wording: every string the control can show is in the new `text:` option - `empty:`, `create:`, `min_chars:`, `progress:`, `error:` and `more_error:` - merged over the defaults in `Layered::Ui::ComboboxHelper::COMBOBOX_TEXT`, so it can be reworded or translated at the call site. Placeholders use Rails' own `%{name}` syntax (`%{term}`, `%{count}`, `%{shown}`); the character threshold is counted and interpolated when the control renders, the rest where the message is built. `progress: nil` drops the progress line, and an unknown key raises rather than going unnoticed.
- Combobox reordering: `reorder: true` gives each token a pair of move controls and makes tokens draggable with a mouse, with parameters posted in the displayed order. A draggable token carries a decorative grip handle and a grab cursor, so the drag is discoverable rather than hidden. The move buttons are the accessible path (dragging alone would fail WCAG 2.2 SC 2.5.7, and they are the only route on a touch screen).
- Combobox remote options: pass `url:` and the options are fetched from that endpoint as the user types, rather than being rendered up front and filtered in the browser - for collections too large to send with the page. Requests are debounced and a response overtaken by a later one is discarded, so the list always answers what is currently typed. `collection:` becomes optional, `min_chars:` sets how much has to be typed before the first search, and remote selections are passed as `["Label", value]` pairs since the browser has no collection to look a label up in. Matches load a page at a time: reaching the end of the list appends the next page, by scrolling to its foot or by pressing Down on its last option, so the keyboard is not capped at the first page. A presentational note (`l-ui-combobox__notice`) sticks to the foot of the list and says how far into the matches it has got, or that they could not be loaded; each page appended is announced. A request in flight shows a spinner (`l-ui-combobox__spinner`) instead - trailing the field while searching, beside the note while a page loads - after a short delay, so a quick answer never flashes one, with `aria-busy` on the listbox for the same state.
