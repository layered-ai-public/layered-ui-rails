...
The canonical framework-agnostic CSS source is `packages/layered-ui-css/src/layered-ui.css`. Rails-only integration styles live in `app/assets/tailwind/layered/ui/rails.css`. The Rails gem vendors both into `app/assets/tailwind/layered/ui/styles.css`; run `bundle exec rake layered_ui:sync_css` after changing the package CSS, Rails CSS, or fonts.
